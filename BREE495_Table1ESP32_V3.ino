...
bool thresholdSwitch = false; //false = can't change the threshold, true = can change the threshold

// Moisture threshold used in automatic mode (percent 0-100)
float weightThreshold = 30.0; // default — can be updated via MQTT
...
void controlSolenoid(float avgMoisture) {
  ...
  if (avgMoisture < weightThreshold) {
    solenoidState = true;
  } else {
    solenoidState = false;
  }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // switch to automatic mode (use existing threshold)
    ...
    Serial.println("Threshold switch is set to be true");
  }

  else if (message.equalsIgnoreCase("false")) {
    thresholdSwitch = false;
    Serial.println("Threshold switch is set to be false");
  }

  // if numeric payload -> set threshold and enable automatic mode
  else if (isNumericString(message)) {
    if (thresholdSwitch == false){
      return;
    }
    else {
    ...
    Serial.println("Moisture threshold set via MQTT to: " + String(weightThreshold, 1) + "% (automatic mode)");
    }
  }
  else {
...
void loop() {
  ...
  if (nau1.available()) {
    long raw1 = nau1.read();
    grams1 = (raw1 - offset[0]) / scale_factor[0]; //need to get the offset and the scale_factor from the deep memory of the ESP32
  }
