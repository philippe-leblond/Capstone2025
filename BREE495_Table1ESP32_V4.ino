...
// Weight threshold used in automatic mode (in grams)
float weightThreshold = 550.0; // default, can be updated via MQTT
float hysteresis = 10.0; //+- 5 buffer around threshold
...
// Automatic control uses the single threshold: if avgWeight < threshold -> ON, else OFF
void controlSolenoid(float avgWeight) {
  if (manualOverride) {
    // manual mode: do not change solenoidState here (it's controlled by MQTT manual commands)
    digitalWrite(solenoidValve, solenoidState ? HIGH : LOW);
    return;
  ...
  // Automatic behaviour using the single threshold provided via MQTT or default
  float onThreshold = weightThreshold - hysteresis / 2;
  float offThreshold = weightThreshold + hysteresis / 2;
  if (avgWeight < onThreshold) {
    solenoidState = true;
  } else if (avgWeight > offThreshold) {
    solenoidState = false;
  }
...
void publishData(float temp, float hum, float m1, float m2, float m3, float avgMoisture, float grams1, float grams2, float grams3, float avgWeight) {
  ...
  payload += "\"weightThreshold\":" + String(weightThreshold, 1) + ",";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // switch to automatic mode (use existing threshold)
    ...
    Serial.println("Solenoid set to automatic mode (using threshold = " + String(weightThreshold, 1) + "g ");
  }
  // if numeric payload -> set threshold and enable automatic mode
    ...
    Serial.println("Weight threshold set via MQTT to: " + String(weightThreshold, 1) + "g (automatic mode)");
  }
  else {
